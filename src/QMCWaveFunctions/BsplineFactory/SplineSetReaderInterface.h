 ...
 */
#ifndef QMCPLUSPLUS_SPLINESET_READER_H_INTERFACE
#define QMCPLUSPLUS_SPLINESET_READER_H_INTERFACE
...
struct SplineSetReaderInterface : public BsplineReaderInterface
{
  ...
  SplineSetReaderInterface(EinsplineSetBuilderInterface* e)
      : BsplineReaderInterface(e), spline_r(NULL), spline_i(NULL), bspline(0), FFTplan(NULL)
  {}

  ~SplineSetReaderInterface() { clear(); }
  ...
  {
    ReportEngine PRE("SplineSetReaderInterface", "create_spline_set(spin,SPE*)");
    ...
    if (!havePsig)
    {
      APP_ABORT("SplineSetReaderInterface needs psi_g. Set precision=\"double\".");
    }
    ...
    if (foundspline)
    {
      ...
      app_log() << "  SplineSetReaderInterface bcast the full table " << now.elapsed() << " sec." << std::endl;
      app_log().flush();
    }
    else
    {
      ...
      if (havePsig) //perform FFT using FFTW
      {
        ...
        app_log() << "  SplineSetReaderInterface initialize_spline_pio " << now.elapsed() << " sec" << std::endl;
...
/*    int Nbands            = bandgroup.getNumDistinctOrbitals();
  ...
  */
  ...
  void initialize_spline_psi_r(int spin, const BandInfoGroup& bandgroup)
  {
    ...
    APP_ABORT("SplineSetReaderInterface initialize_spline_psi_r implementation not finished.");
    int nx = MeshSize[0];
    int ny = MeshSize[1];
    int nz = MeshSize[2];
    splineData_r.resize(nx, ny, nz);
...
//    hdf_archive h5f(&band_group_comm, false);
    Vector<std::complex<double>> cG(mybuilder->Gvecs[0].size());
    for(int i=0;i<mybuilder->Gvecs[0].size();i++)
      cG[i] = (0.0,0.0);
    const std::vector<BandInfo>& cur_bands = bandgroup.myBands;
