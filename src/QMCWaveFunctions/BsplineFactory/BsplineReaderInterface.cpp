...
/** @file BsplineReaderInterface.cpp
 *
...
{
BsplineReaderInterface::BsplineReaderInterface(EinsplineSetBuilderInterface* e)
    : mybuilder(e), MeshSize(0), checkNorm(true), saveSplineCoefs(false)
{
  myComm = mybuilder->getCommunicator();
}

void BsplineReaderInterface::get_psi_g(int ti, int spin, int ib, Vector<std::complex<double>>& cG)
{
  int ncg = 0;
  if (myComm->rank() == 0)
  {
...
BsplineReaderInterface::~BsplineReaderInterface() {}
...
void BsplineReaderInterface::setCommon(xmlNodePtr cur)
{
...
SPOSet* BsplineReaderInterface::create_spline_set(int spin, xmlNodePtr cur, SPOSetInputInfo& input_info)
{
  if (spo2band.empty())
    spo2band.resize(mybuilder->states.size());
...
/** build index tables to map a state to band with k-point folidng
   ...
   */
void BsplineReaderInterface::initialize_spo2band(int spin,
